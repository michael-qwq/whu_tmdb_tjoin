...
public class InsertImpl implements Insert {
     ...
     */
    private Integer insert(int classId, @NonNull List<String> columns, Tuple tuple, int l, int[] index) throws TMDBException {
        SelectImpl select=new SelectImpl(memConnect);
        ...
        if(idlist.size()!=0){
            ...
            for (int i = 0; i < idlist.size(); i++) {
                //得到当前代理类的id
                int deputyId=idlist.get(i);
                //由于基于轨迹相似度的代理类需要两个源类，需要拿到另一个源类，这里使用getAnotherDeputy方法拿到
                int anotherDeputyId=getAnotherDeputy(deputyId,classId);
                //通过select的getTable（classid）方法拿到另一个源类的所有tuple
                TupleList tupleList=select.getTable(anotherDeputyId);//tupleList就是另一个源类的所有tuple
                //遍历另一个源类的所有tuple
                for (int j = 0; j < tupleList.tuplelist.size(); j++) {
                    //拿到另一个源类的当前tuple
                    Tuple tuple2=tupleList.tuplelist.get(j);
                    ...
                    if(similarity>=1.0){
                        ...
                        memConnect.getBiPointerT().biPointerTable.add(new BiPointerTableItem(classId, tupleid, deputyId, i1));
                    }
                }
            }
        }
